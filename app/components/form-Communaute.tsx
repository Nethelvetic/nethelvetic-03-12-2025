...
const FormCommunaute: React.FC = () => {
  const {
    ...
    defaultValues: {
      noBullshit: true, // valeur par défaut pour la checkbox
    },
  });
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 p-4 max-w-md mx-auto">
            ...
            <div className="flex items-center">
              <input
                type="checkbox"
                {...register("noBullshit")}
                defaultChecked={true} // optionnel si déjà défini dans defaultValues
                className="h-4 w-4 text-blue-600 border-gray-300 rounded"
              />
              <label className="ml-2 block text-sm md:text-xl">
                Faire partie de la communauté No BullSH*T
              </label>
            </div>
