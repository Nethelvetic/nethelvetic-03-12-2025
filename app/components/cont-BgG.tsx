...
const ContanerHBg: React.FC<ContanerHBgProps> = ({ children }) => {
  ...
  return (
    //-------------------------1 DEBUT CONTAINER PRINCIPALE
      <div
        ref={containerRef}
        className={`min-h-64 md:min-h-96 max-w-5xl mx-auto rounded-lg bg-bgGardient1 ${isVisible ? "animate-slide-in-slow" : "animate-fade-out-slow"}`}>

        {/*--------1.1 DEBUT CONTENEUR texte/children   */}
        <div className="flex flex-col md:flex-row items-center p-3 md:p-5  md:p-10rounded-lg">
          {children}
        </div>
      </div>
